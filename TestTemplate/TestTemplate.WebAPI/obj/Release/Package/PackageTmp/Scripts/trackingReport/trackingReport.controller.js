(function () {
    ...
    TrackingReportCtrl.$inject = ['$q', '$scope', '$http', 'config', 'authService', '$stateParams', 'kendoGridService','$location'];

    function TrackingReportCtrl($q, $scope, $http, config, authService, $stateParams, kendoGridService,$location) {
      ...
      //  $scope.tenant = tenant;
		...
		function specialChar(string,replacing) {
            ...
            return string.replace("/", replacing)
			.replace("\"", replacing)
            .replace("&", replacing)
			//.replace(":", replace)
            .replace("\\", replacing)
		...
		function getTable1ExportFileName(){
			var strCustomer = $('#select_customer option:selected').text();
            var strKitType = $("#input-customerkittype").val();
			...
			month = String(month);
			//console.log(year);
			year = String(year).substring(2,year.length);
			//console.log(year);
			
			var strdt = day + month + year;
			
			strCustomer = specialChar(strCustomer,"-");
			strKitType = String(strKitType).replace("\"", "inch");
			strKitType = specialChar(strKitType,"-");
			
			return strCustomer+"-"+strKitType+"-"+strdt+".xlsx";
		...
		function getTable2ExportFileName(){
			var strCustomer = $('#select_customer option:selected').text();
            var strKitType = $("#input-customerkittype").val();
			 var descr = $('#select_detail option:selected').text();
			...
			strKitType = String(strKitType).replace("\"", "inch");
			strKitType = specialChar(strKitType,"-");
			descr = String(descr).replace("\"", "inch");
			descr = specialChar(descr,"-");
			
			
			 
			return strCustomer+"-"+strKitType+"-"+descr+"-"+strdt+".xlsx";
		...
		var outputname = getTable1ExportFileName();
		//alert(outputname);
           ...
           //var base64 = dataURL.split(";base64,")[1];
            ...
            //$.post("10.104.100.8:12210/Content", {
            //    base64: base64,
        ...
        function getAllWODetail(descr) {
            ...
            $q.all(promiseArray).then(function (response) {
                ...
                $q.all(promiseArray1).then(function (response) {
                     console.log("getAllWODetail checkparentwoid11", response);
                    ...
                    $q.all(promiseArray2).then(function (response) {
                           console.log("getAllWODetail loop1 result", response);
                        ...
                        $q.all(promiseArray3).then(function (response) {
                             console.log("getAllWODetail loop1-2", response);
                                        ...
                                        $q.all(promiseArray6).then(function (response) {
                                            ...
                                            $q.all(promiseArray9).then(function (response) {
                                                ...
                                                $q.all(promiseArray7).then(function (response) {
                                                    console.log("response7 getIncomDef1", response);
                                                    //to check:line750
                                                    var defect = "";
        ...
        function checkmeasurement(descr) {
            ...
            $q.all(promiseArray).then(function (response) {
                ...
                $q.all(promiseArray5).then(function (response) {
                     console.log("checkmeasurement buildData response21", response);
                        ...
                        $q.all(promiseArray2).then(function (response) {
                            ...
                            $q.all(promiseArray3).then(function (response) {
                                ...
                                for (var i0 = 0; i0 < response.length; i0++) {
									...
									var tempoperationName = tempResponse[i0].data.result[0]["operationName"];
                                        ...
                                        for (var i2 = 0; i2 < nopts; i2++) {
                                            ...
                                            for (var i3 = 0; i3 < $scope.parameter.data.result.length; i3++) {
                                                if (String(paraid).trim() == String($scope.parameter.data.result[i3]["parameterID"]).trim()) {
													...
													if (String(tempoperationName).trim() == String($scope.parameter.data.result[i3]["operationName"]).trim()) {
                                                        isValid = true;
                                ...
                                $q.all(promiseArray4).then(function (response) {
									  ...
									  for(var i =0; i<response.lenth;i++){
										  console.log("debug checkmeasurement8888111", response[i].config)
									  if(response[i].config.data['WOID'] == '2017ATS0030576'){
										  
										   console.log("debug checkmeasurement8888", response)
									  }
									  }
		...
		var filename = getTable2ExportFileName();
        ...
        function searchInNameData(name, nameData) {
            for (var i = 0; i < nameData.length; i++) {
               // for (var j = 0; j < nameData[i].length; j++) {
                    if (nameData[i]['operationName'] == name) {
                        return i;
                    }
               // }
            }
        ...
        function buildData(response, paraidArray, descr, data) {
            ...
            for (var p = 0; p < paraidArray.length; p++) {
                var paraidtemp = paraidArray[p];
				console.log("testh1 paraidtemp",paraidtemp);
                for (var p1 = 0; p1 < $scope.promiseArray1Data.length; p1++) {
                    //  if (String(paraidtemp).trim() == String($scope.promiseArray1Data[p1].data.result[0]["paramterID"]).trim()) {
                    if (String(paraidtemp).trim() == String($scope.promiseArray1Data[p1].data.result[0]["opid"]).trim()) {
                        paraidName.push($scope.promiseArray1Data[p1].data.result[0]["operationName"]);
            ...
            var nameData = [];
			console.log("testh1 promoseArray1",$scope.promiseArray1Data);
            for (var i = 0; i < uniqueNames.length; i++) {
                nameData[i] = [];
                nameData[i]["value"] = [];
                nameData[i]["operationName"] = uniqueNames[i];
                ...
                for (var j = 0; j < $scope.promiseArray1Data.length; j++) {
                    if (String($scope.promiseArray1Data[j].data.result[0]["operationName"]).trim() == name) {
                       // var id = (String($scope.promiseArray1Data[j].data.result[0]["opid"])).trim();  // change2018
					   var id = (String($scope.promiseArray1Data[j].data.result[0]["operationName"])).trim();
                        var isExist = false;
                        for (var k = 0; k < nameData[i].length; k++) {
                            if (nameData[i]["value"][k] == id) {
                                isExist = true;
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < $scope.promiseArray1Data.length; j++) {
                    ...
                    var operationName = String($scope.promiseArray1Data[j].data.result[0]["operationName"]).trim();//opid

                    var paraid = searchInNameData(operationName, nameData);
            ...
            for (var k = 0; k < response.length; k++) {
                ...
                if (response[k].data.result.length != 0) {
                    ...
                    var paraid1 = String(response[k].data.result[0]["operationName"]).trim();
                    var unit = response[k].data.result[0]["unit"];

                    var operationName = searchInNameData(paraid1, nameData);
                    ...
                    if (data[index]["parameterid" + operationName] == "") {
                        data[index]["parameterid" + operationName] = paraid1;
                    }

                    if (data[index]["unit" + operationName] == "") {
                        data[index]["unit" + operationName] = unit;
                    }
                    if (data[index]["result" + operationName] == "") {
                        data[index]["result" + operationName] = result;
                        //todo: check min and max
                    }
                    if (data[index]["max" + operationName] == undefined) {
                        data[index]["min" + operationName] = parastr;
                        data[index]["max" + operationName] = parastr;
                    } else {
                        if (parseFloat(data[index]["max" + operationName]) < parseFloat(parastr)) {
                            data[index]["max" + operationName] = parastr;
                        }
                    }


                    if (data[index]["min" + operationName] == undefined) {
                        ...
                        if (parseFloat(data[index]["min" + operationName]) > parseFloat(parastr)) {
                            // console.log("min reduce", parastr + " " + index);
                            data[index]["min" + operationName] = parastr;
                        }
                    }

                    if (data[index]["ave" + operationName] == "") {
                        data[index]["ave" + operationName] = avg;
                    }
            ...
            for (var k1 = 0; k1 < response.length; k1++) {
                ...
                if (response[k1].data.success && response[k1].data.result.length != 0) {
                   

                    var parastr = response[k1].data.result[0]["value"];

                    var paraid1 = String(response[k1].data.result[0]["operationName"]).trim();

                    var paraid = searchInNameData(paraid1, nameData);
                    //  var unit = response[k].data.result[0]["unit"];
					if(response[k1].data.result[0]['woid'] == "2017ATS0030576"){
						 console.log("buildData loop", response[k1].data.result[0]);
						 console.log("buildData paraid: "+paraid+"nameData: "+nameData+"paraid1: "+paraid1);
					}
