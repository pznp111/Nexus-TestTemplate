(function () {
    ...
    wotrackingCtrl.$inject = ['$q', '$scope', '$http', 'config', 'tenant', 'authService', '$stateParams', 'kendoGridService', '$state'];

    function wotrackingCtrl($q, $scope, $http, config, tenant, authService, $stateParams, kendoGridService, $state) {
        ...
        $scope.preprocessConfirmControl = false; // to control prescrap
        ...
        function load() {
           ...
           // $("#updateReceiveModal").toggle("show");
            ...
            $scope.preprocessConfirmControl = false;
            ...
            $q.all(promiseArrayOperatorName).then(function (response) {
                ...
                if (response.length != 0 && response[0].data.success && response[0].data.result.length != 0 && response[0].data.result[0]["firstName"] != undefined) {
                    ...
                    $("#wotracking-table3-loginName").val(response[0].data.result[0]["firstName"]);
            ...
            $("#wotracking-table3-operatorName").val(authService.currentUser.userName);
            $("#wotracking-table3-loginName").val($scope.OperatorName);
            ...
            if (keyEvent.which === 13){
               // alert('I am an alert');
                ...
                if (input != "" && !isNaN(input)) {
                    ...
                    $("#alertBoxContent").text("Please enter correct work order, only numeric input allowed");
                    $('#alertBox').modal('show');
        ...
        $scope.SaveCompleteOK = function(){
            ...
            if (config.paraSkipWOTrackingPrompt) {
                ...
                $.confirm({
                    title: 'Confirm!',
                    content: "Confirm to update completed qty?",
                    buttons: {
                        confirm: function () {
        ...
        function SaveCompleteOKCase10(newReceived) {
            ...
            if (newReceived <= $scope.OutstandingQty) {
              ...
              //  console.log("SaveCompleteOKCase10 CompletedQty", $scope.CompletedQty);
                ...
                $("#updateReceiveContent").hide();
                $("#saveCompleteContent").show();
                $('#updateReceiveModal').modal('toggle');
        ...
        function UpdateReceivedOKCase10(newReceived) {
            ...
            if ((config.AllowExceedQty && newReceived >= $scope.PreviousRecQty && $scope.ProcOpSeq == 1) ||
            ...
            } else if (newReceived < $scope.PreviousRecQty) { // prescrap
                ...
                // getElementById("WIP-td7_1").style.display = null;

                //if(String(document.getElementById("WIP-td7_1").innerHTML).trim() != ""){
        ...
        $scope.preprocessConfirm = function () {
            $scope.preprocessConfirmControl = true;
            ...
            if ($scope.ScrapQty == 0 || $scope.ScrapQty == "") { // to check to update frmPreProcessScrap.cs line 159 loops
                $("#alertBoxContent").text("Please enter the correct scrap qty");
                $('#alertBox').modal('show');
                ...
                //INSERT INTO [TS_Scrap] ( WOID,WorkCenter,RouteID, OpSeq,ProcOpSeq, 
                //ScrapQty,Remark,ScrapType,ScrapDate,UserID,UserName,Status ) 
                //VALUES ( @WOID, @WorkCenter, @RouteID, @OpSeq, @ProcOpSeq, 
                //@ScrapQty,@Remark, @ScrapType, @ScrapDate, @UserID,@UserName, @Status ) 
                var proprocessremark = String($("#select_preprocessremark-input").val()).trim();
                    promiseArray1.push(
                    ...
                    $q.all(promiseArray1).then(function (response) {
                        ...
                        //UPDATE [TS_WorkOrder] 
                        //SET AccumulatedScrapQty = (AccumulatedScrapQty + @AccumulatedScrapQty), 
                        //AccumulatedScrapDate = @AccumulatedScrapDate, 
                        //--OutstandingQty = (OutstandingQty- @OutstandingQty), 
                        ...
                        $q.all(promiseArray2).then(function (response) {
                            ...
                            if (response.length != 0 && response[0].data.success) {
                                $("#alertBoxContent").text("Pre-process unaccountable scrap successfully added!");
                                $('#alertBox').modal('show');
        ...
        $scope.preprocessClose = function () {
            //fix ui bug
            $("#fix0").show();
            $("#fix1").show();
            $("#fix2").show();
        ...
        $scope.setupStart = function () {
            ...
            //alert("prod3");
            //alert("call setup start");

            console.log("btn clicked setup start", $scope.selectedWOID);
            if ($scope.selectedWOID == undefined || $scope.selectedWOID == "") {
                        ...
                        if ($scope.fpSpreadQueuingWOList.length != 0) {
                            for (var i = 0; i < $scope.fpSpreadQueuingWOList.length; i++) {
                                if (String($scope.fpSpreadQueuingWOList['woStatus']).trim() == "ProcessingStart") {
                                    $("#alertBoxContent").text("Another work order is processing on the same machine. ");
                                    $('#alertBox').modal('show');
                ...
                if (response.length != 0) {
                    if (response[0].data.success && response[0].data.result.length != 0) {
                        ...
                        if (response[0].data.result[0]['accumulatedScrapQty'] != null && response[0].data.result[0]['accumulatedScrapQty'] != 0) {
                            ...
                            //$("#WIP-tr-7").css("display", "block");
                            //$("#WIP-tr-7").removeProp("display");
                            $("#WIP-tr-7").show();
                        } else {
                            //  alert("hide");
        ...
        //'Remark    :
        //'*******************************************************************
        $scope.preprocessremarkClick = function () {
            var remark = String($('#select_preprocessremark option:selected').text()).trim();
            $("#select_preprocessremark-input").val(remark);
        }
        ...
        //'Remark    :
        //'*******************************************************************
        $scope.scrapremarkClick = function () {
            var remark = String($('#select_wotracking-scrap-remark option:selected').text()).trim();
            $("#select_wotracking-scrap-remark-input").val(remark);
        }
        ...
        function case10() {
            ...
            if (String($scope.McType).trim().toLowerCase() == "inhouse" || String($scope.McType).trim().toLowerCase() == "automated") {
        ...
        function CalculateDuration() {
            ...
            $q.all(promiseArray1).then(function (response) {
                ...
                if (response.length != 0 && response[0].data.success && response[0].data.result.length != 0) {
                    ...
                    console.log("calculateDuration mctype", $scope.McType);
                    if ($scope.McType == "Subcon") {
            ...
            $q.all(promiseArray1).then(function (response) {
                ...
                if (response.length != 0 && response[0].data.success && response[0].data.result.length != 0) {
                    ...
                    if (response[0].data.result[0]['sumScrapQty'] != null &&
                        ...
                        $("#WIP-tr-3").show();
                        $("#WIP-tr-3").removeAttr("display");
                        document.getElementById("WIP-td3_1").innerHTML = response[0].data.result[0]['sumScrapQty'];
                        document.getElementById("WIP-td3_2").innerHTML = String(response[0].data.result[0]['latestScrapDate']).replace("T", " ");;
                    } else {
       ...
       // }
        ...
        function CheckWOSubconOpnStatusAssign() {
            ...
            if (fnGetUserAccessRight(String($scope.OperatorName).trim(), "TS_WOTracking")) {
                        ...
                        if ($scope.fnTrackSetupConfigVar == true || config.BypassSetup) {
                            ...
                            if (response[0].data.result[0]['setupStartDate'] == null && response[0].data.result[0]['prodEndDate'] == null) {
                                ...
                                if(config.AllowMultipleOperator){
                                    ...
                                    //Select Distinct StartType,StartDateTime,
                                    //Reason,OperatorID, OperatorName 
                                    //From TS_Operation where WOID = @WOID 
                                    //And WorkCenter  = @WorkCenter
                                    //And ProcOpSeq  = @ProcOpSeq 
                                    //And StopDateTime is NULL 
                                    //And McType Not like 'QC%' COLLATE SQL_Latin1_General_CP1_CI_AS 
                                    //And McType not like 'Subcon%'COLLATE SQL_Latin1_General_CP1_CI_AS 
                            ...
                            if ($scope.WOGlobalWOOpnState == 1)    //Setup not start
                            {
                               ...
                               // alert("test3");
                                ...
                                if ($scope.fnTrackSetupConfigVar == true || config.BypassSetup) { //if bypass swop btns
                                    console.log("WOGlobalWOOpnState not start but bypass setup");
                                    //$("#setup-row-1").hide();
                                    //$("#setup-row-6").hide();
                                    ...
                                    $("#setup-row-6").hide();
                                    $("#production-row-2").show();
                                    //$("#production-row-2").css("display", "block");
                                    //alert("test4");
                                }
                              ...
                              //  $("#select_wotracking-pausereason").val("");
                               ...
                               // $("#select_wotracking-pausereason").val("");
                                ...
                                $("#wo-tracking-updatereceive").css("display", "none");
                                $("#wo-tracking-update").css("display", "block");
                                $("#select_wotracking-pausereason-input").attr('disabled', 'disabled');
                                $("#select_wotracking-pausereason").attr('disabled', 'disabled');
                                //document.getElementById("wo-tracking-onhold").display = "block";
                                $("#wo-tracking-onhold").show();
                            } else if ($scope.WOGlobalWOOpnState == 7) {//job end
                                ...
                                if (config.BypassApproval) { // to check tocheck GlobalVar.WOGlobalOnHoldBypassApproved 
                                    $("#wo-tracking-onhold").show();                                  
                                    $("#select_wotracking-pausereason").prop("disabled", false);
                                    $("#select_wotracking-pausereason-input").prop("disabled", false);
                                } else {
                                    $("#wo-tracking-onhold").hide();
                                    $("#select_wotracking-pausereason").prop("disabled", true);
                                    $("#select_wotracking-pausereason-input").prop("disabled", true);
                                }
                                $("#wo-tracking-updatereceive").css("display", "none");
                                ...
                                $("#production-row-5").hide();
                                if (config.BypassApproval) { // to check tocheck GlobalVar.WOGlobalOnHoldBypassApproved        
                                    console.log("WOGlobalWOOpnState loop9.1");
                                    $("#wo-tracking-onhold").show();
                                    $("#select_wotracking-pausereason").prop("disabled", true);
                                    $("#select_wotracking-pausereason-input").prop("disabled", false);
                                    $("#select_wotracking-pausereason-input").val($scope.WOGlobalWOPauseReason);
                                } else {
                                    console.log("WOGlobalWOOpnState loop9.2");
                                    $("#wo-tracking-onhold").hide();
                                    $("#select_wotracking-pausereason").prop("disabled", true);
                                    $("#select_wotracking-pausereason-input").val($scope.WOGlobalWOPauseReason);
                                    $("#select_wotracking-pausereason-input").prop("disabled", true);
        ...
        function makeTable1(data) {
            ...
            $("#wotracking-table2").kendoGrid({
               ...
               // pageable: true,
                ...
                columns: [
                 {
                     field: "index", title: "#", width: 20

                 },
                 {
                     field: "woid", title: "WOID", width: 50

                 },
                 {
                     field: "workCenter", title: "Work Center", width: 50

                 },
                 {
                     field: "partID", title: "Part ID", width: 10

                 },
                 {
                     field: "description", title: "Part Name", width: 50

                 },
                 {
                     field: "woStatus", title: "Status", width: 30

                 },
                 {
                     field: "procOpSeq", title: "ProcOpSeq", width: 30
            ...
            if (OperationType == "Setup") {
                ...
                $q.all(promiseArray1).then(function (response1) {
                    ...
                    //    }
                        ...
                        $q.all(promiseArray3).then(function (response) {
							console.log("CalculateTimeSpan3 time before",SumDuration);
                            ...
                            if (response.length != 0) {
                                if (response[0].data.success) {
                                    ...
                                    //.toString('HH:mm:ss');
                                    var time = secondsToHms(SumDuration);
									console.log("CalculateTimeSpan3 time after",SumDuration);
                                    //todo:display this.txtSetupDurationHr.Text
                                    $("#wotracking-table3-setuptotalduration").val(time);
                                    updataSetupDurationTable()
                                }
        ...
        function setupStartCase6() {
            ...
            if (!config.AllowMultipleWO && (fnMultipleWOEnableConfig() != "NO")) {//if not allowed multiple, need to check whether there is a processing
                ...
                if ($scope.fpSpreadQueuingWOList.length != 0) {
                    for (var i = 0; i < $scope.fpSpreadQueuingWOList.length; i++) {
                        if (String($scope.fpSpreadQueuingWOList['woStatus']).trim() == "ProcessingStart") {
                            $("#alertBoxContent").text("Unable to proceed, please on hold the current processing WO");
                            $('#alertBox').modal('show');
                ...
                if ($scope.fpSpreadQueuingWOList.length != 0) {
                    for (var i = 0; i < $scope.fpSpreadQueuingWOList.length; i++) {
                        if (String($scope.fpSpreadQueuingWOList['woStatus']).trim() == "ProcessingHold") {
                            $("#alertBoxContent").text("Unable to proceed, please follow Work Order priority list.");
                            $('#alertBox').modal('show');
        ...
        function setupStartCase10() {
            ...
            console.log("setupStartCase10 OrderType", $scope.OrderType);
            if (String($scope.OrderType).trim().toLocaleLowerCase() == "assembly") {
                //todo: line 3867 - 3897
                console.log("setupStartCase10.0");
                if (config.AssemblyCheckAtLastOnly) {
                    console.log("setupStartCase10.1");
                    var promiseArray1 = [];
                    promiseArray1.push(
                    ...
                    $q.all(promiseArray1).then(function (response) {
                        ...
                        console.log("setupStartCase10 ProcOpSeq LastProc " + "|" + $scope.ProcOpSeq + "| |" + $scope.LastProcOpSeq + "|");

                        console.log("setupStartCase10 compare", ($scope.ProcOpSeq >= $scope.LastProcOpSeq));
                        var tempProcOpSeq = $scope.ProcOpSeq;
                        var tempLastProcOpSeq = $scope.LastProcOpSeq;
                        if($scope.ProcOpSeq >= $scope.LastProcOpSeq) {
                            setupStartCase12();
                        } else {
                            console.log("setupStartCase10 DEBUG");
							setupStartCase15();
                        }
                ...
                console.log("setupStartCase10.2");
                setupStartCase15();
            }
        }
        ...
        function setupStartCase20_25_30() { // here we skip case30
            ...
            $q.all(promiseArray1).then(function (response) {
                ...
                if (response.length != 0 && response[0].data.success && response[0].data.result.length != 0) {
                    //  if(true){
                    //$("#wotracking-table3-operatorName").val($scope.OperatorName);
                    //$("#wotracking-table3-loginName").val($scope.OperatorName);
                    setupStartCase40();

                } else {
                    $("#alertBoxContent").text("Operator is not assigned to operate this machine.");
                    $('#alertBox').modal('show');
        ...
        function setupPauseCase10() {
            ...
            if (config.strSkipWOTrackingPrompt) {
                ...
                $.confirm({
                    title: 'Confirm!',
                    content: "Confirm to pause setup?",
                    buttons: {
                        confirm: function () {
                            setupPauseCase20();
                        },
        ...
        function productionPauseCase10() {
            ...
            if (config.strSkipWOTrackingPrompt) {
                ...
                $.confirm({
                    title: 'Confirm!',
                    content: "Confirm to pause production?",
                    buttons: {
                        confirm: function () {
                            productionPauseCase20();
                        },
        ...
        function setupResumeCase2() {
            ...
            //var operatorName = String($("#wotracking-table3-operatorName").val()).trim();
            //var password = String($("#wotracking-table3-password").val()).trim();
            //if (!config.BypassScanOperator && operatorName == "" || password == "") {
        ...
        function setupResumeCase6() {
            ...
            if (config.strSkipWOTrackingPrompt) {
                ...
                $.confirm({
                    title: 'Confirm!',
                    content: "Confirm to resume setup?",
                    buttons: {
                        confirm: function () {
                            setupResumeCase10();
                        },
        ...
        function productionResumeCase6() {
            ...
            if (config.strSkipWOTrackingPrompt) {
                ...
                $.confirm({
                    title: 'Confirm!',
                    content: "Confirm to resume production?",
                    buttons: {
                        confirm: function () {
                            productionResumeCase10();
                        },
        ...
        function setupStopCase20() {
            ...
            if ($scope.fnTrackSetupConfigVar == true || config.BypassSetup) { // if bypass setup
                ...
                $.confirm({
                    title: 'Confirm!',
                    content: "Would you like to continue production ? ",
                    buttons: {
                        confirm: function () {
                            setupStopCase40();
                        },
           ...
           $http.post(config.baseUrlApi + 'HMLVTS/setupStartCase40_6', {
            ...
            $q.all(promiseArray1).then(function (response) {
                ...
                $q.all(promiseArray2).then(function (response) {
                    ...
                    fnUpdateOperator("ProcessingStop",3);//multiple operator
                            $q.all(promiseArray5).then(function (response) {
        ...
        function cmdOnHold_ClickCase10() {
            ...
            if (config.strSkipWOTrackingPrompt) {
                ...
                $.confirm({
                    title: 'Confirm!',
                    content: "Confirm to on hold work order?",
                    buttons: {
                        confirm: function () {
                            cmdOnHold_ClickCase20();
                        },
            ...
            $q.all(promiseArray1).then(function (response) {
                ...
                promiseArray2.push(
                   $http.post(config.baseUrlApi + 'HMLVTS/productionPauseCase20_2', {
                       ...
                       'McID': $scope.McID,
                       'McType': $scope.McType,
                       'reason': $scope.holdtype + " - " + String($("#select_wotracking-pausereason-input").val()).trim(),
                       'OperatorID': String(authService.currentUser.userName).trim(),//
                       'OperatorName': String($scope.OperatorName).trim(),//
                       'ShiftID': '0'
                   })
        ...
        function cmdUpdate_ClickCase8() {
            ...
            if ($scope.CompletedQty == $scope.ActualRecQty  - $scope.ScrapScrapQty) { //completed
                ...
                if ($scope.WOGlobalWOOpnState != 7) {
                    ...
                    if (String($scope.McType).trim().toLowerCase() == "inhouse" || String($scope.McType).trim().toLowerCase() == "automated") {
                        $("#alertBoxContent").text("Please stop production before complete the work order.");
                        $('#alertBox').modal('show');
                        //alert("Please stop production before complete the work order.");
                    } else {
                        $("#alertBoxContent").text("Please enter recieved date before complete the work order.");
                        $('#alertBox').modal('show');
        ...
        function cmdUpdateReceived_ClickCase8() {
            ...
            //    cmdUpdateReceived_ClickCase10();
            //}
            console.log("cmdUpdateReceived_ClickCase8.0","|"+$scope.ScrapScrapQty+"|");
            //prescrap
            if ($scope.ScrapScrapQty != undefined  && String($scope.ScrapQty).trim() != "") {
                if($scope.ScrapScrapQty == "" || $scope.ScrapScrapQty == undefined  ){
                    
                }
            }
            console.log("cmdUpdateReceived_ClickCase8 control", $scope.preprocessConfirmControl);
            if ($scope.preprocessConfirmControl) {
                $scope.preprocessConfirmControl = false;

                $scope.ScraScrapQty = $scope.ScrapQty;
                ...
                console.log("cmdUpdateReceived_ClickCase8 ScrapScrapQty", $scope.ScrapScrapQty);
                console.log("cmdUpdateReceived_ClickCase8 CompletedQty", $scope.CompletedQty);


                console.log("cmdUpdateReceived_ClickCase8.1 actrec - scrap", parseInt(String($scope.ActualRecQty)) - parseInt(String($scope.ScrapQty)));

                if (parseInt(String($scope.CompletedQty)) == parseInt(String($scope.ActualRecQty)) - parseInt(String($scope.ScrapQty))) { //completed
                    console.log("cmdUpdateReceived_ClickCase8.1");
                    $scope.WOExecutionStatus = "Completed"; // update status only for completed job
                    cmdUpdateReceived_ClickCase10();
                } else if (parseInt(String($scope.CompletedQty)) > parseInt(String($scope.ActualRecQty)) - parseInt(String($scope.ScrapQty))) { //do more than completed
                    console.log("cmdUpdateReceived_ClickCase8.2");
                ...
                console.log("cmdUpdateReceived_ClickCase8.1 actrec - scrap", parseInt(String($scope.ActualRecQty)) - parseInt(String($scope.ScrapScrapQty)));

                if (parseInt(String($scope.CompletedQty)) == parseInt(String($scope.ActualRecQty)) - parseInt(String($scope.ScrapScrapQty))) { //completed
                    console.log("cmdUpdateReceived_ClickCase8.1");
                    $scope.WOExecutionStatus = "Completed"; // update status only for completed job
                    cmdUpdateReceived_ClickCase10();
                } else if (parseInt(String($scope.CompletedQty)) > parseInt(String($scope.ActualRecQty)) - parseInt(String($scope.ScrapScrapQty))) { //do more than completed
                    console.log("cmdUpdateReceived_ClickCase8.2");
                    $("#alertBoxContent").text("Please enter the correct completed qty. Max qty is " + $scope.ActualRecQty);
                    $('#alertBox').modal('show');
                    // alert("Please enter the correct completed qty. Max qty is " + $scope.ActualRecQty );
        ...
        function cmdUpdateReceived_ClickCase10() {
            ...
            $q.all(promiseArray1).then(function (response) {
                ...
                fnUpdateOperator(1);//multiple operator
                $q.all(promiseArray2).then(function (response) {
        ...
        function cmdUpdate_ClickCase10() {
            ...
            $q.all(promiseArray1).then(function (response) {
                ...
                fnUpdateOperator(2);//multiple operator
        ...
        function cmdConfirm_ClickCase8() {
            ...
            if ($("#receivedDateInput").prop("disabled")) {
                ...
                if (startDate.getTime() > endDate.getTime()) {
                    $("#alertBoxContent").text("Receive date must be later than send out date.");
                    $('#alertBox').modal('show');
        ...
        function cmdConfirm_ClickCase15() {
            ...
            if (!$("#sendDateInput").prop("disabled")) {
                ...
                $.confirm({
                    title: 'Confirm!',
                    content: "Confirm receive work order?",
                    buttons: {
                        confirm: function () {
                            answer = true;
                        },
        ...
        $scope.ScrapConfirm = function () {
            ...
            var scrapRemark = String($("#select_wotracking-scrap-remark-input").val()).trim()
