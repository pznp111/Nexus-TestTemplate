...
namespace TestTemplate.Domain.Service.Implementations
{
    public class QueuingWOsService : IQueuingWOsService
    {
        ...
        public QueryResponse ValidateOperatorName(Messaging.MessagingService.MessagingService request)
        {
            QueryResponse response = new QueryResponse();
            DataTable result;
            try
            {
                result = iStoredQueryRepository.Translate(new QueryFactory().ValidateOperatorName(request));
                response.Success = true;
                response.Result = result;
                response.TotalRecords = result.Rows.Count;
            }
        ...
        public QueryResponse fnMultipleWOEnableConfig(Messaging.MessagingService.MessagingService request)
        {
            QueryResponse response = new QueryResponse();
            DataTable result;
            try
            {
                result = iStoredQueryRepository.Translate(new QueryFactory().fnMultipleWOEnableConfig(request));
                response.Success = true;
                response.Result = result;
                response.TotalRecords = result.Rows.Count;
            }
